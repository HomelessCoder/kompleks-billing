 ...
 *
 * Общие поля.
 * @property-read string $uid Идентификатор пользователя.
 ...
 * @property-read string $ipaccess Доступ в кабинет клиента только с IP адресов учетных записей кабельных агентов.
 * @property-read string $type Тип пользователя: 1 - Юридическое лицо, 2 - Физическое лицо.
 * @property-read string $balance Общий баланс (PRO, конвергентный биллинг).
 * @property-read string $depth Разрешенный размер кредита в р.е. (только для конвергентного режима).
 ...
 * @property-read string $billDelivery Способ доставки счета: 0-курьер, 1-почтой, 2-самостоятельно, 3-другой.
 * @property-read string $category 0-Бюджет, 1-хозрасчет, 2-население, 3-ПБОЮЛ.
 * @property-read string $lastModDate Дата последнего изменения (заполняется автоматически).
 * @property-read string $wrongActive Счетчик неверно введенных карт предоплаты (максимально 10).
 * @property-read string $wrongDate Дата и время последнего неверного ввода карты предоплаты.
 * @property-read string $diplomat 1 - дипломат, 2 - не дипломат.
 * @property-read string $resident 0 - не резидент, 1 - резидент.
 * @property-read string $agrmType Тип договора: 1-клиент(бизнес), 2-дистрибьютор(реселлер), 3-партнер(комиссионер), 4-оператор(взаимодействие), 5-оператор(присоединение), 6-дистрибьютор(агент), 7-поставщик(оператор), 8-клиент(частный), 7-поставщик(не оператор).
 * @property-read string $oksm Код страны абонента по ОКСМ.
 * @property-read string $okato Место заключения договора по ОКАТО.
 * @property-read string $publicAgree Факт согласия абонента на использовании сведений в инф.-справочных системах.
 *
...
final readonly class Account
{
    public function __construct(
        ...
        public string $wrongActive,
        public string $wrongDate,
        public string $diplomat,
        public string $resident,
        public string $agrmType,
        public string $oksm,
        public string $okato,
        public string $publicAgree,
    ) {
    }
}
